#!/usr/bin/env bash
#
# dotfiles doctor — the harness for driving this host-based dotfiles repo.
#
...
#
# Commands:
#   doctor.sh [doctor] [REPO]        full read-only health check (default REPO = repo this script lives in)
...
# --- locate the repo this script is committed inside -----------------------
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_REPO="$(cd "${SKILL_DIR}/../../.." && pwd)"   # <repo>/.claude/skills/run-dotfiles -> <repo>
...
do_doctor() {
  ...
  repo="$(cd "$repo" 2>/dev/null && pwd)" || { echo "no such repo: $1" >&2; exit 1; }
...
do_worktree() {
  ...
  repo="$(cd "$repo" 2>/dev/null && pwd)" || { echo "no such repo: $2" >&2; exit 1; }
